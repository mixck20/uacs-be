...
// User: Upload receipt image for certificate confirmation
  ...
  try {
    ...
    const certificate = await MedicalCertificate.findByIdAndUpdate(
      id,
      {
        ...
        status: 'issued' // Mark as issued once receipt is uploaded
      },
      { new: true}
    ).populate('userId', 'name email')
     .populate('issuedBy', 'name')
    ...
    // Create notification for clinic staff to confirm receipt
    const clinicStaff = await User.find({ role: { $in: ['clinic_staff', 'admin'] } });
    const notificationPromises = clinicStaff.map(staff => {
      const notification = new Notification({
        userId: staff._id,
        type: 'general',
        title: 'Medical Certificate Receipt Upload',
        message: `${certificate.patientId.fullName} has uploaded a receipt for their medical certificate request`,
        data: {
          certificateId: certificate._id,
          patientName: certificate.patientId.fullName
        }
      });
      return notification.save();
    });
...
// Clinic: Confirm receipt image for certificate
exports.confirmReceiptImage = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (!certificate.receiptImage) {
      return res.status(400).json({ message: 'No receipt image to confirm' });
    }

    certificate.receiptConfirmedAt = new Date();
    certificate.receiptConfirmedBy = req.user.id;
    await certificate.save();
    ...
    if (certificate.userId && certificate.userId._id) {
      await Notification.create({
        userId: certificate.userId._id,
        type: 'general',
        title: 'Medical Certificate Confirmed',
        message: 'Your medical certificate receipt has been confirmed by the clinic.',
        data: {
          certificateId: certificate._id
        }
      });
    ...
    res.json({
      message: 'Receipt image confirmed successfully',
      certificate
    });
  } catch (error) {
